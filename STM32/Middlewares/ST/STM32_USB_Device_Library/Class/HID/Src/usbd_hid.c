...
static uint8_t USBD_HID_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum); //EB new
...
USBD_ClassTypeDef  USBD_HID =
{
  ...
  USBD_HID_DataOut, /*DataOut*/ //EB new
  NULL, /*SOF */
  NULL,
  NULL,
...
#else
  0x02,
#endif         /*nInterfaceProtocol : 0=none, 1=keyboard, 2=mouse*/
...
#else
__ALIGN_BEGIN static uint8_t KBD_ReportDescriptor[HID_KBD_REPORT_DESC_SIZE] __ALIGN_END =
 {
...
/**
  ...
  * @retval status
  */
static uint8_t rx_buf; //EB New

static uint8_t  USBD_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
	uint8_t ret = 0; //EB new
  /* Open EP IN */
  USBD_LL_OpenEP(pdev, HID_EPIN_ADDR, USBD_EP_TYPE_INTR, HID_EPIN_SIZE);
  pdev->ep_in[HID_EPIN_ADDR & 0xFU].is_used = 1U;

  USBD_LL_OpenEP(pdev, HID_EPOUT_ADDR, USBD_EP_TYPE_INTR, HID_EPOUT_SIZE); //EB new
  pdev->ep_out[HID_EPOUT_ADDR & 0xFU].is_used = 1U; //EB new?

  pdev->pClassData = USBD_malloc(sizeof(USBD_HID_HandleTypeDef));

  USBD_LL_PrepareReceive(pdev, HID_EPOUT_ADDR, &rx_buf, 1); //EB new
